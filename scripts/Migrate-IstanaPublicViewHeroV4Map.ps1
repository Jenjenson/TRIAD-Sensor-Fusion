...
function Assert-SameSnapshot {
    ...
    if ($Before.Count -ne $After.Count) {
        throw 'CRITICAL: protected v1/v2/v3 maps/assets, imported hero assets, or renderer settings changed file count during hero-v4 map migration.'
    }
    foreach ($entry in $Before.GetEnumerator()) {
        if (-not $After.ContainsKey($entry.Key) -or $After[$entry.Key] -ne $entry.Value) {
            throw "CRITICAL: protected state changed during hero-v4 map migration: $($entry.Key)"
        }
    }
}
...
$sourceMap = Join-Path $resolvedProject 'Content\Maps\Istana_PublicView_Exterior_v3.umap'
$destinationMap = Join-Path $resolvedProject 'Content\Maps\Istana_PublicView_Exterior_v4.umap'
$invariantReport = Join-Path $resolvedProject 'Saved\TRIAD\IstanaPublicViewV4\Istana_PublicView_Exterior_v4.invariants.json'
...
foreach ($requiredPath in @(
    ...
    $protectedV2Map,
    $sourceMap,
    (Join-Path $geometryRoot 'validate_hero_v4.py'),
    ...
    (Join-Path $heroDiskRoot 'Building\SM_IstanaPublicViewV4_Building_Hero.uasset'),
    $materialDiskRoot
)) {
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "Required hero-v4 migration input is missing: $requiredPath"
    }
}
if (Test-Path -LiteralPath $destinationMap) {
    throw "Refusing to overwrite existing v4 destination map: $destinationMap"
}
...
try {
    ...
    if ($assetValidation.ReturnValue -ne $true) {
        throw "Hero-v4 assets are missing or invalid: $($assetValidation.OutReport)"
    }

    $migrationResponse = Invoke-HeroV4RemoteCall `
        -Endpoint $endpoint `
        -ObjectPath $objectPath `
        -FunctionName 'MigrateIstanaPublicViewExteriorMapToHeroV4' `
        -Parameters @{} `
        -TimeoutSeconds 1800
    if ($migrationResponse.ReturnValue -ne $true) {
        throw "MigrateIstanaPublicViewExteriorMapToHeroV4 failed: $($migrationResponse.OutMessage)"
    }

    $mapValidation = Invoke-HeroV4RemoteCall `
        -Endpoint $endpoint `
        -ObjectPath $objectPath `
    ...
    if ($mapValidation.ReturnValue -ne $true) {
        throw "Post-migration hero-v4 map validation failed: $($mapValidation.OutReport)"
    }
}
catch {
    $operationError = $_
...
if ($null -ne $operationError) {
    if (Test-Path -LiteralPath $destinationMap -PathType Leaf) {
        throw "A new v4 destination exists after a failed migration; inspect it and the invariant report before retrying. $operationError"
    }
...
if ($protectedV1MapShaAfter -ne $protectedV1MapShaBefore -or
    ...
    $report.schema -ne 'triad.istana_public_view_hero_v4.component_invariants.v1' -or
    ...
    $report.sourceMapPackage -ne '/Game/Maps/Istana_PublicView_Exterior_v3' -or
    $report.destinationMapPackage -ne '/Game/Maps/Istana_PublicView_Exterior_v4' -or
    ...
    $report.sourceHeroMesh -ne '/Game/TRIAD/IstanaPublicViewV3/Building/SM_IstanaPublicViewV3_Building_Hero.SM_IstanaPublicViewV3_Building_Hero' -or
    $report.destinationHeroMesh -ne '/Game/TRIAD/IstanaPublicViewV4/Building/SM_IstanaPublicViewV4_Building_Hero.SM_IstanaPublicViewV4_Building_Hero' -or
    ...
    throw 'The persisted Hero-V4 invariant report does not prove exact V1/V2/V3 map bytes, hero-only mutation, components, cameras, and no renderer changes.'
}

[PSCustomObject]@{
    Operation = 'MigrateIstanaPublicViewExteriorMapToHeroV4'
    Succeeded = $true
    ...
    SourceMap = '/Game/Maps/Istana_PublicView_Exterior_v3'
