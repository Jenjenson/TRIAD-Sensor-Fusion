...
$heroV2LibraryObjectPath = '/Script/TRIADSensorFusionEditor.Default__TRIADIstanaPublicViewHeroV2EditorLibrary'
...
function Assert-ExactVisualAcceptanceSettings {
    ...
    if ($item.Length -ne $expectedSettingsBytes -or $digest -cne $expectedSettingsSha256) {
        throw "The project Hero-V3 visual-acceptance settings are not the exact repository-owned no-lidar/RPC-off profile: $SettingsPath"
    }
    ...
    if ([regex]::Matches($text, '(?m)^\s*"RpcEnabled"\s*:').Count -ne 1 -or
        ...
        throw 'Hero-V3 visual acceptance requires the exact non-production ComputerVision profile, RpcEnabled=false and deployed-AirSim compatibility EnableRpc=false exactly once each, loopback API address, and no lidar.'
    }
    ...
    if ($vehicleProperties.Count -ne 1) {
        throw 'Hero-V3 visual acceptance requires exactly one ComputerVision camera vehicle.'
    }
    ...
    if ((Get-RequiredJsonProperty $vehicle 'VehicleType') -cne 'ComputerVision' -or
        ...
        throw 'The sole Hero-V3 visual-acceptance vehicle must be auto-created ComputerVision with an empty Sensors object.'
    }
    return $digest
}
...
function Assert-ExactHeroV3EditorCommandLine {
    ...
    if ([regex]::Matches($CommandLine, $projectPattern).Count -ne 1 -or
        ...
        throw 'The launched Hero-V3 editor command line does not contain exactly one TRIAD project, one canonical V3-only -settings argument, and one visual-acceptance flag. Close it manually before retrying.'
    }
    return $true
}
...
if ($RemoteControlUrl.Scheme -cne 'http' -or
    ...
    throw 'Hero-V3 Remote Control must use exact loopback endpoint http://127.0.0.1:30010.'
}
...
$v2MapFile = Join-Path $projectDirectory 'Content\Maps\Istana_PublicView_Exterior_v2.umap'
$v3MapFile = Join-Path $projectDirectory 'Content\Maps\Istana_PublicView_Exterior_v3.umap'
$heroV3Header = Join-Path $projectDirectory 'Plugins\TRIADSensorFusion\Source\TRIADSensorFusionEditor\Public\TRIADIstanaPublicViewHeroV3EditorLibrary.h'
foreach ($requiredPath in @($settingsPath, $v2MapFile, $heroV3Header)) {
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
        throw "Required Hero-V3 editor-launch input is missing: $requiredPath"
    }
}
...
if ($runningEditors.Count -ne 0) {
    ...
    throw "Close every Unreal Editor before the guarded Hero-V3 launch. Running PID(s): $pids"
}

$v3MapPresent = Test-Path -LiteralPath $v3MapFile -PathType Leaf
$selectedMapFile = if ($v3MapPresent) { $v3MapFile } else { $v2MapFile }
$selectedMapPackage = if ($v3MapPresent) {
    '/Game/Maps/Istana_PublicView_Exterior_v3'
}
else {
    '/Game/Maps/Istana_PublicView_Exterior_v2'
}
$launchMode = if ($v3MapPresent) {
...
if ($identity.ReturnValue -ne $true) {
    throw "Connected Remote Control belongs to the wrong project. No Hero-V3 operation ran. $($identity.OutReport)"
}
...
if ($v3MapPresent) {
    ...
    $mapValidation = Invoke-RemoteObjectCall `
        -Endpoint $callEndpoint `
        -ObjectPath $heroV2LibraryObjectPath `
        -FunctionName 'ValidateIstanaPublicViewHeroV2Map' `
        -TimeoutSeconds 900
}
if ($mapValidation.ReturnValue -ne $true) {
    throw "The launched editor is not on the exact required map. No Hero-V3 mutation ran. $($mapValidation.OutReport)"
}
...
foreach ($key in $protectedHashesBefore.Keys) {
    if ($protectedHashesBefore[$key] -cne $protectedHashesAfter[$key]) {
        throw "Guarded Hero-V3 editor launch changed protected disk bytes: $key"
    }
}
...
$currentMapValidation = if ($v3MapPresent) {
