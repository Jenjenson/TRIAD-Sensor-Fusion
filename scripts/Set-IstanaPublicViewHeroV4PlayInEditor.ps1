...
function Assert-ExactHeroV4EditorCommandLine {
    ...
    if ([regex]::Matches($CommandLine, $projectPattern).Count -ne 1 -or
        ...
        throw 'Hero-V4 PIE start/stop requires exactly one TRIAD .uproject argument, one canonical -settings=<Project>\Config\IstanaPublicViewHeroV3VisualAcceptance.settings.json argument, and one -TRIADIstanaVisualAcceptance flag.'
    }
    return $true
}

function Assert-ExactHeroV4VisualAcceptanceProfile {
    param(
    ...
    if ($item.Length -ne $expectedSettingsBytes -or $digest -cne $expectedSettingsSha256) {
        throw 'Hero-V4 PIE start/stop refuses a settings file other than the exact repository-owned shared Hero-V3 no-lidar/RPC-off profile.'
    }
    ...
    if ([regex]::Matches($text, '(?m)^\s*"RpcEnabled"\s*:').Count -ne 1 -or
        ...
        throw 'Hero-V4 PIE start/stop requires ComputerVision, RpcEnabled=false and deployed-AirSim compatibility EnableRpc=false exactly once each, loopback API address, no DefaultSensors, and no lidar declaration.'
    }
    ...
    if ($vehicleProperties.Count -ne 1) {
        throw 'Hero-V4 PIE start/stop requires exactly one visual-acceptance vehicle.'
    }
    ...
    if ((Get-RequiredJsonProperty $vehicle 'VehicleType') -cne 'ComputerVision' -or
        ...
        throw 'Hero-V4 PIE start/stop requires one auto-created ComputerVision vehicle with an empty Sensors object.'
    }

    $editors = @(Get-CimInstance Win32_Process -Filter "Name='UnrealEditor.exe'")
    if ($editors.Count -ne 1) {
        throw "Hero-V4 PIE start/stop requires exactly one guarded Unreal Editor process; found $($editors.Count)."
    }
...
if ($RemoteControlUrl.Scheme -cne 'http' -or
    ...
    throw 'Hero-V4 PIE Remote Control must use exact loopback endpoint http://127.0.0.1:30010.'
}
$visualProfile = Assert-ExactHeroV4VisualAcceptanceProfile `
    -ProjectDirectory $projectDirectory `
    -ProjectFile $projectFile
$endpoint = [uri]::new($RemoteControlUrl, '/remote/object/call')
$heroLibraryObjectPath = '/Script/TRIADSensorFusionEditor.Default__TRIADIstanaPublicViewHeroV4EditorLibrary'
$runtimeLibraryObjectPath = $heroLibraryObjectPath
$levelEditorObjectPath = '/Script/LevelEditor.Default__LevelEditorSubsystem'
$destinationMap = '/Game/Maps/Istana_PublicView_Exterior_v4'
...
if ($identity.ReturnValue -ne $true) {
    throw "Hero-v4 Remote Control project verification failed: $($identity.OutReport)"
}
...
if ($Stop) {
    ...
    $quiesce = Invoke-RemoteObjectCall `
        ...
        -FunctionName 'QuiesceIstanaPublicViewHeroV4PlayWorldForStop' `
        -TimeoutSeconds 30
    if ($quiesce.ReturnValue -ne $true) {
        throw "Refusing unsafe EditorRequestEndPlay because hero-v4 AirSim quiescence was not verified. PIE was intentionally left running. $($quiesce.OutMessage)"
    }
    ...
    if ($inPlay) {
        throw 'EditorRequestEndPlay did not stop hero-v4 public-view PIE within 30 seconds.'
    }
...
if ($mapValidation.ReturnValue -ne $true) {
    throw "PIE start requires the exact validated Hero-V4 map with V3 surroundings preserved: $($mapValidation.OutReport)"
}
...
if (-not $initiallyInPlay) {
    ...
    if (-not $inPlay) {
        throw 'EditorRequestBeginPlay did not enter hero-v4 public-view PIE within 30 seconds.'
    }
}
...
do {
    ...
    try {
        $playReadiness = Invoke-RemoteObjectCall `
            -ObjectPath $runtimeLibraryObjectPath `
            -FunctionName 'ValidateIstanaPublicViewHeroV4PlayWorldReadiness' `
            -TimeoutSeconds 60
...
if (-not $playReadiness -or $playReadiness.ReturnValue -ne $true -or
    ...
    throw "Hero-v4 PIE readiness did not pass for $requiredStableReadinessPolls consecutive polls within $ReadinessWaitSeconds seconds. PIE was intentionally left running; retry readiness or use this script with -Stop for guarded teardown. Last report: $lastReport"
}

[PSCustomObject]@{
    Operation = if ($sessionWasAlreadyRunning) {
        'ValidateExistingHeroV4PublicViewPlaySession'
    }
    else {
    ...
    ProjectIdentityVerified = $true
    EditorMapValidation = 'ValidateIstanaPublicViewHeroV4Map'
    PlayWorldReadiness = 'ValidateIstanaPublicViewHeroV4PlayWorldReadiness'
