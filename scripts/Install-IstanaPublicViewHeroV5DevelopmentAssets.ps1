<#
.SYNOPSIS
Installs only the additive Istana Public View Hero-V5 development layer.

.DESCRIPTION
Hero-V5 never installs or upgrades Hero-V4 implicitly. A fresh project must
first run Install-IstanaPublicViewHeroV4DevelopmentAssets.ps1 as a separate
reviewed step, import and migrate the V4 assets/map, and close Unreal Editor.
This installer byte-proves the V4 source and shared visual-profile prerequisites,
requires every shared/V4 plugin prerequisite to be current or an explicitly
reviewed read-only-compatible digest, and requires the expected imported V4
map, mesh, material roster, plus the inherited 57 V3 texture package-name
roster before it mutates any Hero-V5 path. Imported
UAsset payload/semantic authority is deliberately deferred to the V5 editor
Import/Migrate gates before the corresponding V5 package save. The installer
publishes validated V5 geometry/material source first and the four explicit V5
plugin files last.
#>
[CmdletBinding()]
param(
...
$expectedBaseInstallerSha256 = 'f042a5257c6759d22bca4092594c8e28a175347c39d09dba14ab36f77542391a'
$expectedBaseInstallerBytes = 40382
...
function Assert-SameSnapshot {
    ...
    if ($Before.Count -ne $After.Count) {
        throw 'CRITICAL: protected V1/V2/V3/V4 content, settings, or shared plugin state changed file count during the additive Hero-V5 installation layer.'
    }
    foreach ($entry in $Before.GetEnumerator()) {
        if (-not $After.ContainsKey($entry.Key) -or $After[$entry.Key] -cne $entry.Value) {
            throw "CRITICAL: protected V1/V2/V3/V4 content, settings, or shared plugin state changed during the additive Hero-V5 installation layer: $($entry.Key)"
        }
    }
}
...
function Get-InstallSourceFiles {
    ...
    return @(Get-ChildItem -LiteralPath $SourceRoot -Recurse -File -Force | Where-Object {
        ...
        $_.FullName -notmatch '[\\/]HeroMaterialsV[345]_Determinism_[a-z0-9_]+(?:[\\/]|$)' -and
...
function Assert-NoGeneratedPythonCache {
    ...
    if ($cacheDirectories.Count -ne 0 -or $cacheFiles.Count -ne 0) {
        ...
        throw "Refusing Hero-V5 installation because $Label contains generated Python cache state: $paths"
    }
}

function Assert-NoHeroV5GeneratedPythonCache {
    param(
        [string] $TargetRoot,
        [string] $Label
    )
    ...
    $cacheFiles = @(Get-ChildItem -LiteralPath $TargetRoot -Recurse -File -Force |
        Where-Object {
            $_.Name -match '^test_istana_public_view_hero_v5_integration_contract\..*\.py[co]$'
        })
    if ($cacheFiles.Count -ne 0) {
        $paths = @($cacheFiles | ForEach-Object FullName) -join ', '
        throw "Refusing Hero-V5 installation because $Label contains generated Hero-V5 Python cache state: $paths"
    }
}

function Assert-NoHeroMaterialsV5DeterminismTemporaryTree {
    param([string] $SourceRoot)
    ...
    $temporaryDirectories = @(Get-ChildItem -LiteralPath $SourceRoot -Recurse -Directory -Force |
        Where-Object { $_.Name -match '^HeroMaterialsV[345]_Determinism_[a-z0-9_]+$' })
    if ($temporaryDirectories.Count -ne 0) {
        $paths = @($temporaryDirectories | ForEach-Object FullName) -join ', '
        throw "Refusing Hero-V5 installation while a material determinism temporary tree exists: $paths"
    }
}
...
function Assert-ExactBaseInstaller {
    ...
    if ($item.Length -ne $expectedBaseInstallerBytes -or $digest -cne $expectedBaseInstallerSha256) {
        throw "The Hero-V5 layer requires the exact reviewed Hero-V4 prerequisite installer; its bytes differ: $InstallerPath"
    }
    return $digest
}
...
function Assert-ExactSharedHeroV3VisualAcceptanceSettings {
    ...
    if ($item.Length -ne $expectedVisualSettingsBytes -or $digest -cne $expectedVisualSettingsSha256) {
        throw "Hero-V5 must reuse the exact frozen Hero-V3 visual-acceptance settings bytes: $SettingsPath"
    }
    ...
    if ([regex]::Matches($text, '(?m)^\s*"RpcEnabled"\s*:').Count -ne 1 -or
        ...
        throw 'The shared Hero-V3 visual-acceptance profile semantics changed; Hero-V5 installation is forbidden.'
    }
    return $digest
}
...
function Invoke-FailClosedSourceValidation {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "ASSETS_MISSING_OR_INVALID: Hero-V5 geometry/freeze validation failed. $($geometryOutput -join [Environment]::NewLine)"
    }
    ...
    $materialOutput = @(& $PythonExecutable -B `
        ...
        --validate-only 2>&1)
    if ($LASTEXITCODE -ne 0) {
        throw "ASSETS_MISSING_OR_INVALID: HeroMaterialsV5 contract/interface/freeze validation failed. $($materialOutput -join [Environment]::NewLine)"
    }
}

function Invoke-FailClosedV4PrerequisiteValidation {
    param(
        [string] $PythonExecutable,
        [string] $GeometryRoot,
        [string] $MaterialRoot
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "V4_PREREQUISITE_INVALID: reviewed Hero-V4 geometry/freeze validation failed. $($geometryOutput -join [Environment]::NewLine)"
    }
    $materialOutput = @(& $PythonExecutable -B `
        (Join-Path $MaterialRoot 'validate_hero_materials_v4.py') `
        --validate-only 2>&1)
    if ($LASTEXITCODE -ne 0) {
        throw "V4_PREREQUISITE_INVALID: reviewed HeroMaterialsV4 validation failed. $($materialOutput -join [Environment]::NewLine)"
    }
}
...
function Assert-NewOrEqualHashFile {
    ...
    if ($sourceHash -cne $destinationHash) {
        throw "Refusing to overwrite a different existing Hero-V5 file: $DestinationFile"
    }
}
...
function Copy-NewOrEqualHashFile {
    ...
    $stageFile = "$DestinationFile.hero_v5_add_$([Guid]::NewGuid().ToString('N')).tmp"
    try {
        ...
        if ($stageHash -cne $sourceHash) {
            throw "Hero-V5 additive staging hash mismatch: $SourceFile"
        }
        try {
            ...
            if (Test-Path -LiteralPath $DestinationFile -PathType Leaf) {
                ...
                throw "Refusing to overwrite a different file created during Hero-V5 additive publication: $DestinationFile"
            }
            throw $publishError
        }
        ...
        if ($installedHash -cne $sourceHash) {
            throw "Installed Hero-V5 file hash mismatch: $DestinationFile"
        }
        return 'ADDED'
    }
    finally {
...
function Assert-NoUnexpectedTargetFiles {
    ...
    if ($unexpected.Count -ne 0) {
        throw "$Label contains files outside the exact reviewed Hero-V5 source roster: $($unexpected -join ', ')"
    }
}
...
function Assert-ExactInstalledPrerequisiteFile {
    ...
    if (-not (Test-Path -LiteralPath $DestinationFile -PathType Leaf)) {
        throw "Hero-V5 requires an exact installed prerequisite ($Label): $DestinationFile. Run Install-IstanaPublicViewHeroV4DevelopmentAssets.ps1 separately, import/migrate V4, close Unreal Editor, and retry."
    }
    ...
    if (-not $accepted.Contains($destinationHash)) {
        throw "Hero-V5 requires an exact current or explicitly reviewed-compatible prerequisite ($Label); target bytes differ: $DestinationFile"
    }
...
function Assert-ExactInstalledPrerequisiteTree {
    ...
    if (-not (Test-Path -LiteralPath $TargetRoot -PathType Container)) {
        throw "Hero-V5 requires the separately installed exact $Label tree: $TargetRoot"
    }
    ...
    Assert-NoUnexpectedTargetFiles `
        ...
        -Label "Installed $Label prerequisite tree"
...
function Assert-ExactInstalledHeroV3TextureRoster {
    ...
    if (-not (Test-Path -LiteralPath $TargetTextureRoot -PathType Container)) {
        throw "Hero-V5 requires the separately imported Hero-V3 texture namespace: $TargetTextureRoot"
    }
    ...
    if ($actualNames.Count -ne 57 -or
        ...
        throw "Hero-V5 requires the exact 57-name frozen-contract Hero-V3 texture package roster and no differently named texture package; offline payload semantics remain editor-gated: $TargetTextureRoot"
    }
    return $actualNames.Count
}

function Assert-ExactInstalledHeroV4MaterialRoster {
    param([string] $TargetMaterialRoot)

    if (-not (Test-Path -LiteralPath $TargetMaterialRoot -PathType Container)) {
        throw "Hero-V5 requires the separately imported HeroMaterialsV4 namespace: $TargetMaterialRoot"
    }
    $expectedRelative = @(
        'M_IPV_HeroSurface_V4.uasset',
        'M_IPV_HeroGlass_V4.uasset',
        'MI_IPV_Hero_Render_V4.uasset',
        'MI_IPV_Hero_Trim_V4.uasset',
        'MI_IPV_Hero_Slate_V4.uasset',
        'MI_IPV_Hero_Louvre_V4.uasset',
        'MI_IPV_Hero_Stone_V4.uasset',
        'MI_IPV_Hero_Timber_V4.uasset',
        'MI_IPV_Hero_PaintedMetal_V4.uasset',
        'MI_IPV_Hero_Glass_V4.uasset',
        'MI_IPV_Hero_Opaline_V4.uasset',
        'MI_IPV_Hero_Recess_V4.uasset'
    ) | Sort-Object
    $actualRelative = @(
        Get-ChildItem -LiteralPath $TargetMaterialRoot -Recurse -File -Filter '*.uasset' -Force |
            ForEach-Object { $_.FullName.Substring($TargetMaterialRoot.Length).TrimStart('\') } |
            Sort-Object
    )
    if ($actualRelative.Count -ne $expectedRelative.Count -or
        [string]::Join("`n", $actualRelative) -cne [string]::Join("`n", $expectedRelative)) {
        throw "Hero-V5 requires the exact 12-object imported HeroMaterialsV4 package roster and no unknown package: $TargetMaterialRoot"
    }
    return $actualRelative.Count
}

if (Get-Process UnrealEditor -ErrorAction SilentlyContinue) {
    throw 'Close every Unreal Editor process before installing Hero-V5 plugin/source assets.'
}

$repositoryRoot = Split-Path -Parent $PSScriptRoot
$baseInstaller = Join-Path $PSScriptRoot 'Install-IstanaPublicViewHeroV4DevelopmentAssets.ps1'
$sourcePluginRoot = Join-Path $repositoryRoot 'unreal\Plugins\TRIADSensorFusion'
$sourceGeometryRoot = Join-Path $repositoryRoot 'unreal\SourceAssets\IstanaPublicViewV5'
$sourceMaterialRoot = Join-Path $repositoryRoot 'unreal\SourceAssets\IstanaPublicView\HeroMaterialsV5'
...
$sourceV4GeometryRoot = Join-Path $repositoryRoot 'unreal\SourceAssets\IstanaPublicViewV4'
$sourceV4MaterialRoot = Join-Path $repositoryRoot 'unreal\SourceAssets\IstanaPublicView\HeroMaterialsV4'
$sourceMaterialFreeze = Join-Path $sourceMaterialRoot 'hero_materials_v5.freeze.json'
$sourceIntegrationFreeze = Join-Path $sourcePluginRoot 'Resources\IstanaPublicViewHeroV5.integration.freeze.json'
...
$targetGeometryRoot = Join-Path $resolvedProject 'SourceAssets\IstanaPublicViewV5'
$targetMaterialRoot = Join-Path $resolvedProject 'SourceAssets\IstanaPublicView\HeroMaterialsV5'
...
$targetV4GeometryRoot = Join-Path $resolvedProject 'SourceAssets\IstanaPublicViewV4'
$targetV4MaterialRoot = Join-Path $resolvedProject 'SourceAssets\IstanaPublicView\HeroMaterialsV4'
$targetV4Map = Join-Path $resolvedProject 'Content\Maps\Istana_PublicView_Exterior_v4.umap'
$targetV4Mesh = Join-Path $resolvedProject 'Content\TRIAD\IstanaPublicViewV4\Building\SM_IstanaPublicViewV4_Building_Hero.uasset'
$targetV4MaterialAssetRoot = Join-Path $resolvedProject 'Content\TRIAD\IstanaPublicView\HeroMaterialsV4'

if (-not (Test-Path -LiteralPath $sourceMaterialFreeze -PathType Leaf)) {
    throw "ASSETS_MISSING_OR_INVALID: the reviewed HeroMaterialsV5 freeze is absent. No V4 prerequisite installer or Hero-V5 target mutation ran: $sourceMaterialFreeze"
}

$v5PluginRelativePaths = @(
    'Source\TRIADSensorFusionEditor\Public\TRIADIstanaPublicViewHeroV5EditorLibrary.h',
    'Source\TRIADSensorFusionEditor\Private\TRIADIstanaPublicViewHeroV5EditorLibrary.cpp',
    'Tests\test_istana_public_view_hero_v5_integration_contract.py',
    'Resources\IstanaPublicViewHeroV5.integration.freeze.json'
)
$requiredSourcePaths = @(
    $baseInstaller,
    $projectFile,
    ...
    (Join-Path $sourceGeometryRoot 'validate_hero_v5.py'),
    (Join-Path $sourceGeometryRoot 'hero_v5.freeze.json'),
    (Join-Path $sourceGeometryRoot 'Generated\SM_IstanaPublicViewV5_Building_Hero.obj'),
    (Join-Path $sourceMaterialRoot 'validate_hero_materials_v5.py'),
    $sourceMaterialFreeze,
    $sourceIntegrationFreeze
)
$requiredSourcePaths += @($v5PluginRelativePaths | ForEach-Object { Join-Path $sourcePluginRoot $_ })
foreach ($requiredPath in $requiredSourcePaths) {
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
        throw "Required frozen Hero-V5 installation input is missing: $requiredPath"
    }
}
foreach ($requiredRoot in @(
    ...
    $sourceV4GeometryRoot,
    $sourceV4MaterialRoot
)) {
    if (-not (Test-Path -LiteralPath $requiredRoot -PathType Container)) {
        throw "Required frozen Hero-V3/V4 prerequisite source tree is missing: $requiredRoot"
    }
}
...
Assert-NoHeroMaterialsV5DeterminismTemporaryTree -SourceRoot $sourceMaterialRoot
Assert-NoHeroMaterialsV5DeterminismTemporaryTree -SourceRoot $sourceV3MaterialRoot
Assert-NoGeneratedPythonCache -SourceRoot $sourcePluginRoot -Label 'repository plugin source'
Assert-NoGeneratedPythonCache -SourceRoot $sourceGeometryRoot -Label 'repository Hero-V5 geometry source'
Assert-NoGeneratedPythonCache -SourceRoot $sourceMaterialRoot -Label 'repository HeroMaterialsV5 source'
...
Assert-NoGeneratedPythonCache -SourceRoot $sourceV4GeometryRoot -Label 'repository Hero-V4 geometry prerequisite source'
Assert-NoGeneratedPythonCache -SourceRoot $sourceV4MaterialRoot -Label 'repository HeroMaterialsV4 prerequisite source'
...
if ($forbiddenMaterialPayloads.Count -ne 0) {
    throw "HeroMaterialsV5 must remain reference-only and cannot install texture/package payloads: $(@($forbiddenMaterialPayloads | ForEach-Object FullName) -join ', ')"
}
...
Invoke-FailClosedV4PrerequisiteValidation `
    -PythonExecutable $pythonExecutable `
    -GeometryRoot $sourceV4GeometryRoot `
    -MaterialRoot $sourceV4MaterialRoot
...
$pluginFiles = @($v5PluginRelativePaths | ForEach-Object { Get-Item -LiteralPath (Join-Path $sourcePluginRoot $_) })
if ($geometryFiles.Count -eq 0 -or $materialFiles.Count -eq 0 -or $pluginFiles.Count -ne 4) {
    throw 'ASSETS_MISSING: the exact Hero-V5 additive source or plugin roster is incomplete.'
}

# V5 never runs the V4 installer. Its recursive plugin roster would include the
# current V5 files and could publish them before V5 source validation/copying.
# Instead, byte-prove the separately installed V3/V4 source/profile inputs,
# accept only current or explicitly reviewed-compatible plugin bytes, and
# require its imported package roster before any V5 target mutation. The V5
...
Assert-NoHeroV5GeneratedPythonCache -TargetRoot $targetPluginRoot -Label 'target project plugin'
Assert-NoGeneratedPythonCache -SourceRoot $targetPluginRoot -Label 'target project plugin'
Assert-NoHeroMaterialsV5DeterminismTemporaryTree -SourceRoot $targetV3MaterialRoot
...
Assert-NoGeneratedPythonCache -SourceRoot $targetV4GeometryRoot -Label 'installed Hero-V4 geometry prerequisite source'
Assert-NoGeneratedPythonCache -SourceRoot $targetV4MaterialRoot -Label 'installed HeroMaterialsV4 prerequisite source'
...
$installedV4GeometryFileCount = Assert-ExactInstalledPrerequisiteTree `
    -SourceRoot $sourceV4GeometryRoot `
    -TargetRoot $targetV4GeometryRoot `
    -Label 'Hero-V4 geometry source'
$installedV4MaterialFileCount = Assert-ExactInstalledPrerequisiteTree `
    -SourceRoot $sourceV4MaterialRoot `
    -TargetRoot $targetV4MaterialRoot `
    -Label 'HeroMaterialsV4 material source'
Invoke-FailClosedV4PrerequisiteValidation `
    -PythonExecutable $pythonExecutable `
    -GeometryRoot $targetV4GeometryRoot `
    -MaterialRoot $targetV4MaterialRoot

$v5PluginRelativePathSet = [System.Collections.Generic.HashSet[string]]::new(
    [System.StringComparer]::OrdinalIgnoreCase)
foreach ($relativePath in $v5PluginRelativePaths) {
    [void] $v5PluginRelativePathSet.Add($relativePath)
}
...
$v4BasePluginFiles = @($allRepositoryPluginFiles | Where-Object {
    $relativePath = Get-RelativePath -Root $sourcePluginRoot -FullName $_.FullName
    -not $v5PluginRelativePathSet.Contains($relativePath)
})
if ($v4BasePluginFiles.Count -eq 0) {
    throw 'The exact shared Hero-V4/base plugin prerequisite roster is empty.'
}
$reviewedV4PluginPins = @(
    [PSCustomObject]@{ RelativePath = 'Source\TRIADSensorFusionEditor\TRIADSensorFusionEditor.Build.cs'; Sha256 = '34ba790e50b87d495c2eff41aa4afbc2798af8524dd891740dccab466df973f5'; Bytes = 699 },
    [PSCustomObject]@{ RelativePath = 'Source\TRIADSensorFusionEditor\Public\TRIADIstanaPublicViewHeroV4EditorLibrary.h'; Sha256 = 'bafd7fe66f97bdfa66a1595b462903f5ff943d4138dfecc92f6af355e65db497'; Bytes = 3599 },
    [PSCustomObject]@{ RelativePath = 'Source\TRIADSensorFusionEditor\Private\TRIADIstanaPublicViewHeroV4EditorLibrary.cpp'; Sha256 = '533c719b4ab5566bd10d0b8158ed05952b13f1d20ae2c7e8dbe2f705a8446678'; Bytes = 371716 },
    [PSCustomObject]@{ RelativePath = 'Tests\test_istana_public_view_hero_v4_integration_contract.py'; Sha256 = 'b609f062d570887257e5d8279ed2d2787ee2551d48c9ddd6a0690acb81735ad7'; Bytes = 42345 },
    [PSCustomObject]@{ RelativePath = 'Resources\IstanaPublicViewHeroV4.integration.freeze.json'; Sha256 = 'c6afedad2ca868be455a5425ab02114fb1584d237700c9fba489b595be650f12'; Bytes = 7279 }
)
foreach ($pin in $reviewedV4PluginPins) {
    $pinPath = Join-Path $sourcePluginRoot $pin.RelativePath
    if (-not (Test-Path -LiteralPath $pinPath -PathType Leaf) -or
        (Get-Item -LiteralPath $pinPath).Length -ne [int64] $pin.Bytes -or
        (Get-FileHash -LiteralPath $pinPath -Algorithm SHA256).Hash.ToLowerInvariant() -cne [string] $pin.Sha256) {
        throw "The repository V4 prerequisite escaped its exact independently reviewed pin: $($pin.RelativePath)"
    }
}
...
foreach ($sourceFile in $v4BasePluginFiles) {
    ...
    elseif ($relativePath -ceq $v3DiagnosticCompatibleRelativePath) {
        ...
        # capture diagnostic string. It remains runtime-compatible with V5 and
        # is accepted strictly as a read-only prerequisite; V5 never upgrades
    ...
    elseif ($relativePath -ceq $v3DiagnosticTestCompatibleRelativePath) {
        ...
        # is accepted read-only and is never copied over or upgraded by V5.
...
foreach ($requiredInstalledPackage in @($targetV3Map, $targetV3Mesh, $targetV4Map, $targetV4Mesh)) {
    if (-not (Test-Path -LiteralPath $requiredInstalledPackage -PathType Leaf)) {
        throw "Hero-V5 requires the separately imported/migrated Hero-V3/V4 package prerequisite before installation: $requiredInstalledPackage"
    }
}
...
$installedV4MaterialAssetCount = Assert-ExactInstalledHeroV4MaterialRoster `
    -TargetMaterialRoot $targetV4MaterialAssetRoot
...
Assert-NoUnexpectedTargetFiles `
    ...
    -Label 'Target Hero-V5 geometry source namespace'
Assert-NoUnexpectedTargetFiles `
    -TargetRoot $targetMaterialRoot `
    -ExpectedRelativePaths $materialRelativePaths `
    -Label 'Target HeroMaterialsV5 source namespace'
...
$protectedPaths = @(
    ...
    (Join-Path $resolvedProject 'SourceAssets\IstanaPublicViewV4'),
    ...
    (Join-Path $resolvedProject 'SourceAssets\IstanaPublicView\HeroMaterialsV4'),
...
$protectedPaths += @($v4BasePluginFiles | ForEach-Object {
    $relativePath = Get-RelativePath -Root $sourcePluginRoot -FullName $_.FullName
...
try {
    ...
    # Validate the exact installed V5 source trees before publishing any V5
    # plugin byte. If validation fails, no editor-callable V5 surface exists.
    Assert-NoUnexpectedTargetFiles `
        -TargetRoot $targetGeometryRoot `
        -ExpectedRelativePaths $geometryRelativePaths `
        -Label 'Installed Hero-V5 geometry source namespace'
    Assert-NoUnexpectedTargetFiles `
        -TargetRoot $targetMaterialRoot `
        -ExpectedRelativePaths $materialRelativePaths `
        -Label 'Installed HeroMaterialsV5 source namespace'
    Invoke-FailClosedSourceValidation `
        -PythonExecutable $pythonExecutable `
        -GeometryRoot $targetGeometryRoot `
        -MaterialRoot $targetMaterialRoot

    # These are the only V5 plugin files. They are intentionally published
    ...
    Assert-NoHeroV5GeneratedPythonCache -TargetRoot $targetPluginRoot -Label 'installed target project plugin'
    Assert-NoGeneratedPythonCache -SourceRoot $targetPluginRoot -Label 'installed target project plugin'
    Assert-ExactSharedHeroV3VisualAcceptanceSettings -SettingsPath $targetVisualSettings | Out-Null
}
catch {
    $installError = $_
...
[PSCustomObject]@{
    Operation = 'InstallIstanaPublicViewHeroV5DevelopmentAssets'
    Succeeded = $true
    Project = $resolvedProject
    V4PrerequisiteInstallerCalled = $false
    V4PrerequisiteInstallerSha256 = $baseInstallerSha256
    V3V4SourceAndProfilePrerequisitesByteExact = $true
    V4PluginPrerequisitesCurrentOrReviewedCompatible = $true
    ReviewedV4PluginPinCount = $reviewedV4PluginPins.Count
    ...
    V4PluginPrerequisitesOverwritten = $false
    ImportedV4PackageRosterPresent = $true
    ImportedV3TexturePackageRosterPresent = $true
    ImportedV4SemanticValidationDeferredToV5EditorGate = $true
    FreshProjectRequiresSeparateV4Install = $true
    ...
    InstalledV4GeometrySourceFileCount = $installedV4GeometryFileCount
    InstalledV4MaterialSourceFileCount = $installedV4MaterialFileCount
    InstalledV4BasePluginFileCount = $v4BasePluginFiles.Count
    InstalledV4MaterialPackageCount = $installedV4MaterialAssetCount
    ...
    ExplicitV5PluginFileCount = $pluginFiles.Count
    ...
    V1V2V3V4ProtectedStateMutated = $false
