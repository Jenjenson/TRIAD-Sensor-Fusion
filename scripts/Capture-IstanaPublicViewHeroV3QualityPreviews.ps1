...
$outputDirectory = Join-Path $projectDirectory 'Saved\TRIAD\IstanaPreviews\PublicViewHeroV3'
...
foreach ($mapPath in @($v1MapPath, $v2MapPath, $v3MapPath)) {
...
function Get-StableHeroV3Readiness {
    $lastReadiness = $null
    for ($poll = 1; $poll -le 3; ++$poll) {
        ...
        if ($lastReadiness.ReturnValue -ne $true) {
            throw "Hero-v3 PIE readiness failed on stable poll $poll/3: $($lastReadiness.OutReport)"
        }
        if ($poll -lt 3) {
...
$initialReadiness = Get-StableHeroV3Readiness
$cameraSpecs = @(
    [PSCustomObject]@{
        Preset = 'HERO_FRONT'
        Slug = 'hero_front'
...
foreach ($camera in $cameraSpecs) {
    $outputFileName = "ipv_v3_quality_$($camera.Slug)_$RunId.png"
    $outputPath = Join-Path $outputDirectory $outputFileName
    if (Test-Path -LiteralPath $outputPath) {
        throw "Refusing to overwrite existing hero-v3 preview: $outputPath"
    }

    $response = Invoke-PublicViewCall `
        -ObjectPath $runtimeLibraryObjectPath `
        -FunctionName 'CaptureIstanaPublicViewHeroV3QualityFrame' `
        -Parameters @{
    ...
    if ($response.ReturnValue -ne $true) {
        throw "Hero-v3 PIE capture '$($camera.Preset)' failed: $($response.OutMessage)"
    }
    ...
    if ($stableLengthPolls -lt 2) {
        throw "Hero-v3 capture was scheduled but no stable PNG appeared within 60 seconds: $outputPath"
    }

    $dimensions = Get-PngDimensions -LiteralPath $outputPath
    if ($dimensions.Width -ne 3840 -or $dimensions.Height -ne 2160) {
        throw "Hero-v3 capture has unexpected dimensions $($dimensions.Width)x$($dimensions.Height): $outputPath"
    }
...
$finalReadiness = Get-StableHeroV3Readiness
...
if ($v1HashAfter -ne $v1HashBefore) {
    throw "V1 map changed during hero-v3 capture; refusing acceptance. Before=$v1HashBefore After=$v1HashAfter"
}
if ($v2HashAfter -ne $v2HashBefore) {
    throw "V2 map changed during Hero-V3 capture; refusing acceptance. Before=$v2HashBefore After=$v2HashAfter"
}
if ($v3HashAfter -ne $v3HashBefore) {
    throw "V3 map changed during transient capture; refusing acceptance. Before=$v3HashBefore After=$v3HashAfter"
}

[PSCustomObject]@{
    Operation = 'CaptureIstanaPublicViewHeroV3QualityPreviews'
    Succeeded = $true
