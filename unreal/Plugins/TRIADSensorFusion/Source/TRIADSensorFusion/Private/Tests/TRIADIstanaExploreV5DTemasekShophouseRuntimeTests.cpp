...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FTRIADIstanaExploreV5DTemasekShophouseRuntimeContractTest,
    "TRIAD.Istana.ExploreV5D.TemasekShophouse.RuntimeContract",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FTRIADIstanaExploreV5DTemasekShophouseRuntimeContractTest::RunTest(
    const FString& Parameters)
{
    (void)Parameters;
    const ATRIADIstanaExploreV5DTemasekShophouseActor* Defaults =
        GetDefault<ATRIADIstanaExploreV5DTemasekShophouseActor>();
    TestNotNull(TEXT("R24B Temasek Shophouse class default object"), Defaults);
    if (!Defaults)
    {
        return false;
    }
    ...
    TestFalse(TEXT("Coarse-shell overlap remains explicitly unresolved"),
        Defaults->bCoarseShellOverlapResolved);
    TestFalse(TEXT("Provider overlap remains explicitly unresolved"),
        Defaults->bProviderOverlapResolved);
    ...
    TestEqual(
        ...
        FString(TEXT("/Game/TRIAD/IstanaPublicViewExploreV5D/Surroundings/R24TemasekShophouse/SM_IPV5D_R24_TemasekShophouse_Render.SM_IPV5D_R24_TemasekShophouse_Render")));

    const FTransform Placement =
        ATRIADIstanaExploreV5DTemasekShophouseActor::ExpectedPlacementTransform();
    ...
    TestEqual(
        TEXT("Foliage handoff schema is exact"),
        ATRIADIstanaExploreV5DLandmarkVegetationActor::
            ExpectedTemasekFoliageLayoutSchema(),
        FString(TEXT("triad.istana_explore_v5d.r24_temasek_shophouse.foliage_layout.v1")));
    TestEqual(
        TEXT("Foliage render owner is the dedicated landmark actor"),
        ATRIADIstanaExploreV5DLandmarkVegetationActor::
            ExpectedTemasekFoliageRenderOwnerClass(),
        FString(TEXT("ATRIADIstanaExploreV5DLandmarkVegetationActor")));
    TestTrue(
        TEXT("Foliage source placement equals the building placement"),
        ATRIADIstanaExploreV5DLandmarkVegetationActor::
            ExpectedTemasekSourcePlacementTransform().Equals(
                Placement,
                0.000001));
    const TArray<FVector>& TreeAnchors =
        ATRIADIstanaExploreV5DLandmarkVegetationActor::
            ExpectedTemasekTreeAnchorsLocalMeters();
    TestEqual(TEXT("Exactly three R24 local tree anchors are handed off"),
        TreeAnchors.Num(), 3);
    if (TreeAnchors.Num() == 3)
    {
        TestTrue(TEXT("Umbrella tree anchor is exact"),
            TreeAnchors[0].Equals(FVector(-18.5, -19.0, 0.0), 0.000001));
        TestTrue(TEXT("Dome tree anchor is exact"),
            TreeAnchors[1].Equals(FVector(18.5, -18.8, 0.0), 0.000001));
        TestTrue(TEXT("High-fork tree anchor is exact"),
            TreeAnchors[2].Equals(FVector(-13.2, -16.2, 0.0), 0.000001));
    }
    ...
    ATRIADIstanaExploreV5DTemasekShophouseActor* TransientActor =
        NewObject<ATRIADIstanaExploreV5DTemasekShophouseActor>(
            GetTransientPackage(),
            NAME_None,
            RF_Transient);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FTRIADIstanaExploreV5DTemasekShophouseAssetContractTest,
    "TRIAD.Istana.ExploreV5D.TemasekShophouse.AssetContract",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FTRIADIstanaExploreV5DTemasekShophouseAssetContractTest::RunTest(
    const FString& Parameters)
{
    (void)Parameters;
    const FString& Path =
    ...
    TestNotNull(TEXT("Exact cooked Temasek Shophouse mesh loads"), Mesh);
    if (!Mesh)
    {
        return false;
    }
    ...
    const bool bValid =
        ATRIADIstanaExploreV5DTemasekShophouseActor::ValidateRuntimeMesh(
            Mesh,
            ContractError);
    if (!bValid)
    {
